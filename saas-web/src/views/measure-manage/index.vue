    ...
    <div class="search-header-row search-margin-bottom">
      ...
      <div>
        <el-select v-model="query.quantityKind" filterable clearable placeholder="类别" title="类别">
          <el-option
              v-for="(item, index) in quantityKindList"
              :key="index"
              :label="item.text"
              :value="item.value">
          </el-option>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchEnums() {
        ...
        const { options } = data;
        this.quantityKindList = (options.quantityKind).filter(data=>{ return !data.isDisabled});
      },
