      ...
      <div>
        <Row>
          <Col span="12">
            ...
            <i-switch v-model="isShowErrorList" @on-change="onlyShowErrList"
              ><span slot="open">开</span><span slot="close">关</span></i-switch
            >
            <span style="line-height: 30px; color: red"> (只显示错误行)</span>
          </Col>
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      if (isAccpet) {
        ...
        this.$Message.error(
          `文件:${file.name}格式不正确，请上传.xls或.xlsx文件。`
        );
      }
    ...
    async getSheetData() {
      ...
      if (!sheetData) {
        ...
        if (errMessage.length > 0)
          this.$Modal.error({ title: '错误提示', content: errMessage });
    ...
    onlyShowErrList() {
      if (this.isShowErrorList) {
        this.loadList = this.previewData.resultDatas
          ? this.previewData.resultDatas.filter(
            (data) => data.errorMessage.length > 0
          )
          : [];
      } else {
        this.loadList = this.previewData.resultDatas || [];
    ...
    //     if(this.curLoadIndex < this.previewData.resultDatas.length){
    //       setTimeout(() => {
    //         for(let i=0; i<this.maxLoadData;i++){
    //             if(this.curLoadIndex > this.previewData.resultDatas.length-1){
    //               this.loading = false
    //               return
    //             }else{
    //               this.showList.push(this.previewData.resultDatas[this.curLoadIndex])
    //               this.curLoadIndex++
    //             }

    //         }
    //         this.loading = false
    //         console.log(this.curLoadIndex)
    //           resolve();
    //       }, 500);
    //     }else{
